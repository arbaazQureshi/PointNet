...
class ModelNetSSLDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # list_id2pix => view id, x, y, point_id
        local_imgs = []
        for id2pix in list_id2pix:
            x = id2pix[2]
            y = id2pix[1]
            minx = int(max(0, x - self.window_size / 2))
            maxx = int(min(32, x + self.window_size / 2))
            miny = int(max(0, y - self.window_size / 2))
            maxy = int(min(32, y + self.window_size / 2))
            local_img = list_image[id2pix[0]][:, :, miny:maxy, minx:maxx]
            local_img = torch.nn.functional.pad(local_img, (0, self.window_size - local_img.shape[-1], 1, self.window_size - local_img.shape[-2] - 1), "constant", 0)
        ...
        return point_set, torch.cat(list_image, dim=0), torch.cat(local_imgs, dim=0), list_id2pix
