...
if __name__ == "__main__":
    ...
    print(args)
    # define model
    model = LocalModel(args)
    # model.cuda()
    # freeze feature extractor in 2d networks
    for param in model.net2d.parameters():
        param.requires_grad = False

    if args.use_sgd:
        print("Use SGD")
        optimizer = optim.SGD(
            list(model.net3d.parameters()),
            lr=args.lr * 100,
        ...
        print("Use Adam")
        optimizer = optim.Adam(list(model.net3d.parameters()), lr=args.lr)
    ...
    for epoch in range(1, args.nepoch + 1):
        ...
        for data in tqdm(train_loader, total=len(train_loader)):
            ...
            model(point_cloud, images, local_images, pix2point)
            jnjln
            loss_reg = 0.0
